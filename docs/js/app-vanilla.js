...
</div>`).content;this.$timers=f(t).timers,this.$timers.append(...this.timers),this.append(t);for(let e of["click","change","timerUpdated","clockStarted","clockStopped","timerDeleted"])this.addEventListener(e,this)}handleEvent(t){t.stopPropagation(),t.preventDefault();let e=t.target;if(e instanceof HTMLElement){let i=e.dataset.action||e.closest("[data-action]")?.dataset?.action;i?this[i]instanceof Function?this[i](t):console.error(`Action ${i} not implemented.`):this[t.type]instanceof Function&&this[t.type](t)}}addTimer(){let t={id:Date.now(),hours:0,minutes:0,seconds:0,title:"",sound:null};this.data.timers.push(t);let e=new s(t,this.data);this.timers.push(e),this.$timers?.append(e),this.save()}clockStarted(t){let e=t.target;e instanceof s&&(this.activeTimers.push(e),this.tick())}clockStopped(t){t.target instanceof s&&(this.activeTimers=this.activeTimers.filter(i=>i!==t.target),this.interval&&this.activeTimers.length===0&&(clearInterval(this.interval),this.interval=null))}editSettings(){let t=l("dialogs");if(!t){console.error("Dialogs container not found.");return}this.data.allowedSounds.length===0&&this.data.allowedSounds.push(...o.slice(1).map(([i])=>i));let e=`
<x-dialog>
    <dialog class=modal>
        <div>
...
</x-dialog>`;t.innerHTML=e,l("allowed-sounds")?.addEventListener("change",this)}timerUpdated(t){let e=t.target;if(!(e instanceof s))return;let i=e.timer,n=this.data.timers.findIndex(T=>T.id===i.id);if(n===-1){console.error("Timer not found.");return}this.data.timers[n]=i,this.save()}timerDeleted(t){let e=t.target;if(!(e instanceof s))return;let i=e.timer.id;e.remove(),this.data.timers=this.data.timers.filter(n=>n.id!==i),this.save()}tick(){this.interval||(this.interval=setInterval(()=>{for(let t of this.activeTimers)t.tick()},1e3))}saveAllowedSounds(t){let e=t.target;if(e instanceof HTMLInputElement){this.data.allowedSounds=[];for(let i of e.form?.querySelectorAll("input[type=checkbox]")??[])i instanceof HTMLInputElement&&i.checked&&this.data.allowedSounds.push(i.value);this.save()}}saveOption(t){let e=t.target;if(!(e instanceof HTMLInputElement||e instanceof HTMLSelectElement))return;let i=this.data;switch(e.name){case"sound":i.sound=e.value;break}this.save()}save(){localStorage.setItem(h,JSON.stringify(this.data))}},v=g(`
<div>
<div>
    <label>
...
</div>`),s=class extends HTMLElement{cache;timer;info;startedAt;totalTime;timeoutId;node;constructor(t,e){super(),this.cache=new Map,this.timer=t,this.info=e;let i=v.content.cloneNode(!0),n=f(i);this.node={...n,clockEl:n.stopEl},d(n.title,t.title),d(n.hours,t.hours||""),d(n.minutes,t.minutes||""),d(n.seconds,t.seconds||""),this.appendChild(i),this.addEventListener("click",this),this.addEventListener("change",this),this.stopClock()}handleEvent(t){t.stopPropagation();let e=t.target;if(e instanceof HTMLElement){let i=e.dataset.action;i&&(this[i]instanceof Function?this[i](t):console.error(`Action ${i} not implemented.`))}}save(t){let e=t.target;if(!(e instanceof HTMLInputElement||e instanceof HTMLSelectElement))return;let i=this.timer,n=e.value;switch(e.name){case"hours":case"minutes":case"seconds":i[e.name]=+n;break;case"title":i.title=n;break;case"sound":i.sound=n==="default"?null:n;break}this.sendNotification("timerUpdated")}stopClock(){this.node.stopEl.classList.remove("overlay"),m(this.node.startEl),r(this.node.restartEl),r(this.node.clockSeperator),this.node.alarmEl.textContent="",this.clearClock()}startClock(){r(this.node.startEl),m(this.node.clockSeperator),m(this.node.restartEl),this.setClock(),this.sendNotification("clockStarted"),this.timeoutId=setTimeout(()=>{this.sendNotification("timerExpired")},this.getTotalSeconds()*1e3)}restartClock(){this.stopClock(),this.startClock()}deleteTimer(){this.sendNotification("timerDeleted")}startAlarm(){this.clearClock(),r(this.node.clockSeperator);let t=p(this.timer.sound||this.info.sound,this.info.allowedSounds);if(!t){this.stopClock();return}this.node.alarmEl.innerHTML=t,this.node.stopEl.classList.add("overlay"),clearTimeout(this.timeoutId),this.timeoutId=void 0}clearClock(){this.startedAt=null,this.node.clockEl.textContent="",clearTimeout(this.timeoutId),this.timeoutId=void 0,this.sendNotification("clockStopped")}editSettings(){let t=l("dialogs");if(!t){console.error("Dialogs container not found.");return}let e=`
<x-dialog>
    <dialog class=modal>
        <div>
            <h2 class=inline>Options</h2>
    ...
    ></iframe>`}function l(a){return document.getElementById(a)}function f(a){let t={};for(let e of a.querySelectorAll("[x]"))t[e.getAttribute("x")||""]=e,e.removeAttribute("x");return t}function g(a){let t=document.createElement("template");return t.innerHTML=a,t}function d(a,t){a.value=""+t}
